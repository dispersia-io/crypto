...
describe('Crypto Translator (CommonJS)', () => {
  describe('Initialization checks', () => {
    it('throws if both keys are completely missing', () => {
      expect(() => Crypto.init({ publicKey: '', privateKey: '' })).toThrow(
  ...
  describe('Integration & Output Formatting', () => {
    it('encrypts to base64 and verifies inequality with plaintext', () => {
      const crypto = Crypto.init({
        publicKey: TEST_PUBLIC_KEY,
        privateKey: TEST_PRIVATE_KEY,
      });

      const plainText = 'CommonJS Test Message';
